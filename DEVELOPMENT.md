...
- [x] **Баг 4**: `restoreState` перезаписывает все инструменты сохранённым подмножеством, теряя новые
- [x] **Баг 5**: `/tools handler` не обновляет `enabledTools` из `pi.getActiveTools()`, из-за чего включение выключенных инструментов замещает существующие
- [x] **Баг 6**: `persistTools()` на каждом toggle создаёт множество записей в сессии; при быстром двойном переключении сохраняется только первое состояние. Исправлено: сохранение только при закрытии меню
- [x] **Баг 7**: `getBase()` удалял только один `-` префикс → при многократных сохранениях накапливались `---`. Исправлено: `replace(/^-+/, "")` и `getBase(raw)` при сохранении
- [x] **Баг 8**: `saveSkillList`/`saveExtPackageList` вызывались безусловно, даже без изменений — исправлено
...
4. ~~**[Баг 4]** Починить restoreState/tools handler — ИСПРАВЛЕНО~~
5. ~~**[Баг 6]** persistTools на каждый toggle — ИСПРАВЛЕНО~~
6. **[Тест]** Добавить тесты для всех трёх команд
