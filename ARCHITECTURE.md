...
│   ├── settings-menu.ts  ← общий TUI-builder для persistent-команд
│   ├── extension-menu.ts ← /extensions (делегирует settings-menu.ts)
│   ├── skill-menu.ts     ← /skills (делегирует settings-menu.ts)
│   ├── tool-menu.ts      ← /tools
...
2. Состояние хранится **в двух местах**:
   - **в сессии pi** через `pi.appendEntry()` — для /tree-навигации
   - **в глобальном файле** `~/.pi/agent/tools-config.json` — между сессиями
3. Изменения применяются **мгновенно** через `pi.setActiveTools()`
4. На каждый toggle вызывается `pi.setActiveTools()` **и** `persistTools()`
5. **Приоритет восстановления:** сессия > глобальный конфиг > все включены
...
/tools handler
  ...
  └─ Закрытие TUI
       └─ persistTools()
            ├─ appendEntry в сессию
            └─ запись в ~/.pi/agent/tools-config.json
```
...
### 4.3. settings-menu.ts — общий builder

Начиная с v1.0.0, `/extensions` и `/skills` используют общий
TUI-builder `registerSettingsMenu()` из `lib/settings-menu.ts`.
Он принимает параметры:

| Параметр | Назначение |
|----------|-----------|
| `commandName` | Имя команды ("/extensions", "/skills") |
| `description` | Описание для регистрации |
| `headerTitle` | Заголовок в TUI |
| `emptyMessage` | Уведомление при пустом списке |
| `changedLabel` | Текст "n … toggled" |
| `buildItems` | Функция сборки элементов |
| `saveItems` | Функция сохранения |

Исключён дублированный TUI-цикл (~60 строк × 2).
...
| **Базовый класс** | `registerSettingsMenu()` | `registerSettingsMenu()` | Отдельная реализация |
...
| JSON parse error | Перехватывается, логируется, возвращается `{}` |
...
3. **Сохранение на каждый toggle** — `persistTools()` вызывается
   при каждом переключении инструмента (внутри `onChange`), а не
   только при закрытии меню. Это гарантирует, что состояние
   выживает при краше pi между toggle и закрытием. Повторный
   вызов при закрытии — дополнительная гарантия.

4. **Двойное хранилище: сессия + глобальный файл** — состояние
   инструментов сохраняется одновременно в сессию (`appendEntry`)
   и в глобальный файл `~/.pi/agent/tools-config.json`. Это
   обеспечивает: (а) восстановление при /tree-навигации в рамках
   сессии, (б) сохранение между разными сессиями.

4. **Общий TUI-builder `registerSettingsMenu()`** — `/extensions` и `/skills`
   используют один и тот же TUI-цикл из `settings-menu.ts`. Файлы
   `extension-menu.ts` и `skill-menu.ts` содержат только конфигурацию
   (имена, заголовки, функции сборки/сохранения).

5. **`_localEnabled`** — временный флаг для отслеживания изменений
   в /extensions и /skills. Позволяет сохранять только если
   пользователь действительно что-то поменял.

6. **`getBase(/^-+/)`** — удаляет ВСЕ ведущие дефисы, предотвращая
   накопление `---` при многократных сохранениях.

7. **Сравнение по `getBase()` при слиянии user + project** — поиск
   дубликатов идёт по базовому имени (без префиксов `-`/`+`), а не по
   полной строке. Это предотвращает дублирование элементов, когда
   в user-файле запись включена, а в project — отключена.
